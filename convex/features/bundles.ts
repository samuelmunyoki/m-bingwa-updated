...
export const createBundle = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    console.log("=== CREATE BUNDLE DEBUG ===");
    console.log("isMultiSession:", isMultiSession);
    console.log("responseValidatorText:", responseValidatorText);
    ...
    // If isSimpleUSSD is false, responseValidatorText should be empty
    if (isMultiSession && isSimpleUSSD) {
      return {
        status: "error",
        message: "Bundle cannot be both Multi-session and Simple USSD. Please select only one option.",
